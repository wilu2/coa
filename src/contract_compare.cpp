...
//这个函数用于比较两个文档的文本内容，通过将文档的页面连接成一个完整的文本字符串，
//并对两个文本字符串进行差异比对，得到比对的结果 resutl1 和 resutl2，并统计字符差异信息保存在 char_counter 中。
...
std::string contract_compare(const std::string& docs, RuntimeConfig& config)
{
    ...
    //my_json detail_result = my_json::array();: 这行代码定义了一个 JSON 数组对象 detail_result，用于存储比对的详细结果。
    //这行代码定义了一个字符计数器对象 char_counter，用于统计比对结果中的字符信息。
    my_json detail_result = my_json::array();//定义了一个 JSON 数组对象 detail_result，用于存储比对的详细结果。
    CharCounter char_counter;
    ...
    try
    {
        ...
        //这行代码将传递进来的字符串 docs 解析成 JSON 对象 js，其中包含了两个文档的信息和相关配置。
        my_json js = my_json::parse(docs);

        //: 这行代码从 JSON 对象中获取文档索引 doc_index 的值，用于指示要比对的第一个文档。
        //这行代码根据 doc_index 的值确定另一个文档的索引，以便进行比对。
        int doc_index = js[0]["doc_index"].get<int>();
        int first_doc_index = (doc_index == 1 ? 0 : 1);


        //这行代码设置比对的源文档索引为 first_doc_index。
        src.doc_index = first_doc_index;
        //这行代码设置比对的目标文档索引为 1-first_doc_index，即另一个文档。
        cnd.doc_index = 1-first_doc_index;
        // 在接下来的代码中，通过使用以上定义的 src 和 cnd 对象加载文档的页面和布局，并对文档进行逐页比对，然后生成比对结果。
    ...
    //这行代码定义了一个 JSON 对象 result，用于存储最终的比对结果。
    my_json result;
    //这行代码将比对的详细结果存储在 result 对象中的 "detail" 键下。
    result["result"]["detail"] = detail_result;
    //这行代码将字符相似度存储在 result 对象中的 "similarity" 键下。
    result["result"]["similarity"] = char_counter.GetSimilarity();

    //函数返回将 result 对象转换为字符串形式的 JSON 格式，并进行缩进处理。
