...
int TextLine::init_line(my_json& js, int _page_index, int _line_index, Context& context, int& origin_char_index)
{
    ...
    std::vector<int> polygon = js["position"].get<std::vector<int> >();//从JSON对象js中读取position字段的值，并将其转换为std::vector<int>类型，并存储在polygon变量中。
    

    text = from_utf8(str);
    poly = BoundBox::toBoundBox(polygon);//这行代码调用BoundBox类的静态方法toBoundBox，将polygon向量转换为一个矩形边界框，并赋值给poly成员变量。
